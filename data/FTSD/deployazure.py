...
for name, size in vmlist:
    run(
        f"az vm create --resource-group vm1_group --name {name} --size {size} --image UbuntuLTS --ssh-key-values id_rsa.pub --admin-username ansible"
    )
...
run(
    f"az storage account create --name shreshthstorage --resource-group vm1_group --access-tier Hot --kind StorageV2 --location uksouth --sku Standard_RAGRS"
)
